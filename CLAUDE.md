...
### 8. Git の役割分担・ブランチ戦略（最初にユーザーと合意する）
どこまで AI（Claude Code）が git を操作するか・どうブランチを分けるかは人・案件で異なる。**認識のズレと
勝手なマージを防ぐため、最初のコミットに入る前にユーザーと合意し、その範囲を超えない。** 未合意なら保守的に
倒し、範囲外の操作は実行せず「提案」に留める。この合意は Rule 9（権限ポスチャ）とともに、初期化時の
**作業合意インタビュー**（`orchestrate` 4-5）で一度に決め、`.orchestrator/working-agreement.md` に記録する。

- git の役割（ユーザーに確認する境界の例）:
  - (a) **コミットまで**（push はユーザーが実施）
  - (b) **ブランチ push ＋ PR 作成まで**（マージはユーザーが最終判断）← 未指定時の推奨デフォルト
  - (c) **マージまで自走**（merge / squash の別・ブランチ削除の要否も合わせて合意）
- ブランチ戦略（既定は**機能別**。パイプラインの「1機能ずつ」と一致し PR も機能単位でレビューしやすい）:
  - **機能別（既定）**: バックログ1機能=1ブランチ（`feat/F-XXX-<短い名前>`）
  - **フェーズ/まとめ**: 関連する複数機能を1ブランチ／**単一作業ブランチ**: `work` 1本／**main 直（非推奨・使い捨てのみ）**
...
- 合意した内容（git 役割・ブランチ戦略・その他）は `.orchestrator/working-agreement.md`（人間可読の真実源）と
  `feedback_history`（変更履歴）に記録し、以降その前提で動く。指示が無ければ引き継ぎ、変えたくなったら再合意する。

...
### 全部を使いこなす必要はありません（まず入れるだけでも効きます）

機能はいろいろあります（品質ゲート・敵対的監査・誠実性チェック・セキュリティCI…）。一見すると盛りだくさんに見えるかもしれませんが、**README を全部理解して全機能を使いこなす必要はありません。** 段階的に導入できるよう設計しています。

- **まずは入れるだけ。** 必要なファイル（[オーケストレーション本体](#a-オーケストレーション本体必須)）を自分のプロジェクトに置くだけで、Claude Code が設計⇔コード⇔テストの突合や、**偽成功・エラーの握り潰し・IDOR といったありがちなバグ・問題をある程度自動で洗い出す**ようになります。
- **慣れてきたら足していく。** Claude Code と相談しながら、Gate 3（マージ前レビュー）・敵対的監査・依存の実在性チェック・定期監査…と、**必要な機能を少しずつ有効化**していけます（どれも opt-in か助言的で、いきなり全部を強制しません）。
- その積み重ねで、**受託会社のような品質・セキュリティの証跡**（設計書・テスト・品質/セキュリティレポート）が自然に残るよう設計しています。

迷ったら、まず [クイックスタート](#クイックスタート) の通りに動かして、出てきた成果物を眺めるところから始めてください。

### 特徴

- **ハイブリッド開発プロセス** — 全体はアジャイル（バックログ駆動・機能単位のイテレーション）、1機能の中はウォーターフォール（品質ゲート付きの工程直列）。[詳細](#開発の進め方--アジャイル--ウォーターフォールのハイブリッド)
- **API従量課金なし** — Claude Code（Pro/Maxプラン）の範囲内で完結（任意の定期auditのCI自動化だけは API キー利用＝従量課金）
- **品質ゲート内蔵** — Gate 1（設計レビュー）/ Gate 2（設計⇔コード⇔テスト突合）で自動差し戻し
- **セッション再開** — 中断しても `project_status.yaml` から続きを再開
- **ステークホルダー向け説明資料も自動生成** — 品質レポート・セキュリティ証跡・設計サマリー・プレゼン用スライド（[Gamma](https://gamma.app) 取り込み → PPT/PDF 化対応）を納品・報告・合意形成のときに一括生成（[詳細](#納品ドキュメントステークホルダーへの説明gamma対応)）
...
- **1機能の内側はウォーターフォール（V字モデル）**
  各機能は「設計 → Gate 1 → 実装＋テスト設計 → Gate 2 → レビュー」を必ずこの順で通ります。設計書・コード・テストを品質ゲートで突合するドキュメント駆動なので、受託開発でステークホルダーに見せられる設計書と品質証跡が自然に残ります。納品・報告・合意形成のときには、これらを元にステークホルダー向けの説明資料一式（品質・セキュリティレポート＋プレゼン用スライド）を `delivery` スキルで自動生成できます（後述のオンデマンドスキル）。
...
### 人間（あなた）がやることは実質3つ（＋必要に応じてステークホルダーへの資料共有）

| タイミング | やること |
|---|---|
| 最初 | 要件定義の対話に答え、要件定義書を**承認**する（承認まで先へ進まない）。必要なら顧客と要件を共有して合意する（確認用スライドも生成可） |
| 1機能が完成するたび | 生成されたテスト手順書（`docs/review_map/`）どおりに動作確認する |
| Gate が3回連続 FAIL したとき | エスカレーションを受けて判断する（設計・実装・差し戻しは普段 AI が自走） |
| （任意）納品・報告のタイミング | `delivery` 等で品質/セキュリティレポート・設計サマリー・スライドを生成し、ステークホルダーへ共有する（**生成は AI**。最終確認と共有先の判断が人間） |

上の3つが開発を回している間の作業。ステークホルダーへの資料共有は必要なときだけで（着手前の**要件合意**・納品時の**報告**の2点。どちらも Gamma でスライド化可）、なければ不要。

### 各工程と成果物の対応

| 工程 | Skill | 成果物 |
|---|---|---|
| 0. 要件定義（対話型） | `requirements` | `docs/requirements.md` / `docs/backlog.md`（任意で顧客確認用スライド `docs/requirements_presentation_*.md`） |
...
| `delivery` | ステークホルダー向けの説明資料一式を `docs/delivery/` に集約（上2つ＋設計サマリー＋[Gamma](https://gamma.app) 取り込み用スライド）。任意で共有Webページ（claude.ai Artifact）も生成可（[詳細](#納品ドキュメントステークホルダーへの説明gamma対応)） | 納品・報告・合意形成のとき |
...
> **導入は2通りあります。**
> - **① このリポジトリを clone / フォークして単体で動かす** → 以下の手順（1〜3）。
> - **② 既存プロジェクトに後付けする**（必要なファイルだけダウンロードして自分の PJ に入れる）→ [既存プロジェクトに組み込む](#既存プロジェクトに組み込む) を参照。コピーするファイルを目的別にまとめてあります。
>
> どちらの場合も、下の「2. リポジトリ設定の手動ステップ」は**自分の GitHub リポジトリで同じように**使えます。
...
起動したら、**作りたいものを自分の言葉で伝えるだけ**。技術スタックの提案・機能の洗い出し（F-001…）・抜け漏れの確認は、Claude Code が**対話で一緒に**進めます（要件定義フェーズ）。ガチガチに整理してから書く必要はありません。

```
社内で使う備品予約ツールを作りたいです。
- 社員が備品（プロジェクター・会議室など）を予約できる
- 誰がいつ何を借りているか一覧で見たい
- 同じ時間帯の二重予約は防ぎたい
利用者は社員10人ほど。要件の整理と、技術スタックの提案からお願いします。
```

すると Claude Code が対話で要件を構造化し（機能を F-001, F-002… に分解、受入条件・未決事項も整理）、技術スタックを理由つきで提案します。**あなたが「承認」するまで設計には進みません。** 承認後は 設計 →[Gate 1]→ 実装＋テスト →[Gate 2]→ レビューガイド の順にパイプラインが回り、`docs/` `src/` `tests/` に成果物を生成します。中断しても次回 `前回の続きからお願いします` で再開できます。
...
.claude/hooks/       # 誠実性チェック + 権限ポスチャのフック一式。フォルダごと。★settings.json の hooks で発火させる
.claude/agents/      # 監査用の専門サブエージェント（honesty-auditor / authz-idor-auditor / contract-checker）。フォルダごと
scripts/analyze_l1.py # 観測レシート → allow-list 候補の抽出（任意・単発実行）
```
...
>
> `.claude/hooks/` にはもう一つ、**権限ポスチャ**（どこまで AI に自動承認を委ねるか）の仕組みが入る
> （`policy.py` / `deny_guard.py` / `permission_gate.py` / `l1_shadow_log.py`）。既定は最も安全な
> `conservative`（自動承認ゼロ＋危険操作の deny フロアのみ）で、**入れるだけで素の Claude Code より
> 安全**になる。3ポスチャ（conservative / balanced / permissive）の選び方は
> [`docs/security/PERMISSION_POSTURE.md`](docs/security/PERMISSION_POSTURE.md)。こちらも settings.json の
> `hooks`（下記 B）で発火させる。
>
> **既存プロジェクトへの導入は `scripts/apply_posture.py` で自動化できる**（このリポジトリの checkout から
> `python3 scripts/apply_posture.py /path/to/your-project`）。フック配置・settings.json のマージ
> （**既存の allow・独自フックは温存**）・posture 初期値の作成・レシートの .gitignore 追記までを冪等に行う。
...
| `.claude/settings.json` | 既存の `permissions` があれば `deny`/`ask` 配列を結合。`hooks.PreToolUse` に権限ポスチャの3フック（deny_guard → permission_gate → l1_shadow_log）、`hooks.PostToolUse` に l1_shadow_log と誠実性フックを追加（既存フックがあれば配列に追記） |
...
├── .claude/
│   ├── settings.json                # 権限制御（permissions.deny/ask）＋ フック発火設定（PreToolUse/PostToolUse）
│   ├── hooks/
│   │   ├── honesty_check.py         # 誠実性チェック（偽成功・握り潰しを書いた瞬間に検出）
│   │   ├── policy.py                # 権限ポスチャの真実源: deny フロア regex ＋ 可逆性分類器
│   │   ├── deny_guard.py            # 独立フロア強制（不可逆/外向き/自己権限昇格を常時拒否）
│   │   ├── permission_gate.py       # ポスチャ駆動の自動承認ゲート（conservative/balanced/permissive）
│   │   └── l1_shadow_log.py         # 観測レシート記録（redact 済み・yes を押した実績を蓄積）
...
│   ├── dependabot.yml               # github-actions を SHA ピン追従更新（この repo 自身用）
...
│       └── README.md
├── scripts/
│   ├── analyze_l1.py                # 観測レシート → allow-list 昇格候補の抽出
│   ├── apply_posture.py             # 既存PJへの権限ポスチャ導入（温存マージ・冪等）
│   └── verify_permission_hooks.py   # フック一式の検証ハーネス（保守者向け・46ケース）
...
├── docs/
│   ├── security/
│   │   ├── LEVEL2_SECURITY.md       # セキュリティ構成の解説・手順
│   │   └── PERMISSION_POSTURE.md    # 権限ポスチャ（3段階の自動承認 + 常時 deny フロア）
...
│   └── delivery/              # ステークホルダー向け納品ドキュメント（報告・納品時に生成）
├── src/                       # ソースコード
└── tests/                     # テストコード
```
...
- GitHub Actions（`Security (Level 2)`）でPR時に TruffleHog / Semgrep / Trivy / OSV-Scanner を自動実行し、品質ゲートでマージを制御（いずれも言語自動判定。`pip-audit`(Python) / `composer-audit`(PHP) は対象ファイルが無ければ自動スキップ）
...
- **権限ポスチャ**（どこまで AI に自動承認を委ねるか）をユーザーが選べる。詳細は下記＋[`docs/security/PERMISSION_POSTURE.md`](docs/security/PERMISSION_POSTURE.md)
...
「毎回 yes を押す」か「可逆な操作は自動で進める」かは、人・案件・会社の方針で変わる。**ユーザーが
ポスチャを選び、指示が無ければ前回の選択を引き継ぐ**（CLAUDE.md Rule 9。Git 役割分担の Rule 8 と同じ型）。

| posture | 自動承認 | 向いている人 |
|---|---|---|
| **conservative**（既定） | なし（全部人が承認） | 情報漏洩を防ぎたい企業・慎重派 |
| **balanced** | 可逆・ローカル・読取専用のみ | 手間を減らしたい通常運用 |
| **permissive** | deny フロア以外すべて | リスク受容・使い捨てワークツリー |

- **deny フロアは全ポスチャで常時有効**: `rm -rf` / force push / sudo / `.env`・特権ファイル書込 / `curl|sh` 等の
  不可逆・外向き・自己権限昇格は permissive でも拒否する（だから全許可も責任もって選べる）。
- ポスチャは `.orchestrator/permission_posture.json` に永続化。壊れていれば安全側（conservative）に倒し**明示する**。
- 全ポスチャで操作を観測レシートに記録（秘密は redact 済み・コミット禁止）。溜まったら
  `python3 scripts/analyze_l1.py` で「毎回 yes している可逆コマンド」を allow-list 候補として抽出できる（人が精査して採用）。

### 使い始めるときの手動ステップ

ファイルを置くだけでは完結しない設定がある。自分の GitHub リポジトリ（フォークでも既存プロジェクトでも同じ）で
**最初に1回だけ**実施する。具体的な手順は上の[クイックスタート](#クイックスタート)の
「1. clone してフックを入れる」「2. リポジトリ設定の手動ステップ」と同じ（重複を避けここでは繰り返さない）。
...
**納品・中間報告・合意形成のとき**に `docs/delivery/` へ一括生成できる。パイプライン外の**オンデマンドスキル**として実行する。
...
同じ Gamma 取り込みの手順は**要件定義の段階でも使えます**。`requirements` スキルが任意で生成する
顧客確認用スライド（`docs/requirements_presentation_*.md`）を Gamma に取り込めば、**着手前に顧客と
スコープを合意**できます（受託で最重要の「言った・言わない」防止）。この資料でも未決事項・スコープ外は
隠さず載せます。

## 検証ポイント（フレームワーク自体の動作確認・保守者向け）

> このフレームワーク自体が正しく動くかを確かめるための観点です（利用者の日常操作ではありません）。
...
7. 権限ポスチャのフック一式（決定表・フォールバック・redact・導入スクリプト）:
   `python3 scripts/verify_permission_hooks.py` で自動検証できる（ライブセッション不要・全PASSで exit 0）

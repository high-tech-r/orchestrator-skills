# Skill: security-report（顧客向けセキュリティ証跡レポート生成）

## いつ使うか
- リリース時・納品時に、顧客へ渡すセキュリティ証跡をまとめるとき
...
- 顧客が読む前提で、専門用語には1行の補足を付ける（SBOM＝ソフトウェア部品表 等）。
- 本レポートの構成・表現は `SECURITY.md`（顧客向け説明）と整合させる。
